...
function moving_avg(
    ...
    elseif edge_handling == "extrapolate"
        ...
        fit_left = Polynomials.fit(left_idx,spectrum[left_idx], fit_order);
        fit_right = Polynomials.fit(right_idx,spectrum[right_idx], fit_order)

        spectrum = [
            fit_left.([i for i in -box_size+1:0]);
            spectrum;
            fit_right.([
...
Transforms a matrix of 3 vector tuples into three seperate 3D matrices.
"
function separate_tuple_matrix(
    matrix::Matrix{
        <:Tuple{<:Vector{T1}, <:Vector{T1}, <:Vector{T2}}
    }
)::Tuple{
    Matrix{Vector{T1}},
    Matrix{Vector{T1}},
...
function make3d(im::Array{<:Vector{<:Real},2})
    ...
    return permutedims(
        [
            im[I][k]
            for k=eachindex(im[1,1]), I=CartesianIndices(im)
        ],
        (2,3,1)
    )
end
...
# arr,λ = h5open("D:/SelenoSpecData/M3_data/gruit/region_showrtwvls.hdf5") do f
